...
contract MarketTreasury is Ownable, Lockable {
    ...
    uint256 public liquidFundPercent = 0; // @ you'd better set value only once
    ...
    event SetLiquidFundPercent(uint256 _percent);
    ...
    // team should transfer principle token to fulfill it gap through public market
    ...
    function setLiquidFundPercent(uint256 _percent) external onlyOwner() onlyInitialized() {
        require(_percent > 0 && _percent < 100, "percent value invalid");

        liquidFundPercent = _percent;

        emit SetLiquidFundPercent(_percent);
    }
    ...
    // @note: the liquitity manager also need to be seted as LIQUIDITY DEPOSITOR of Treasury
    ...
    function alphaLiquidFunds() private view returns (uint256) {
        if (!isInitialized || liquidFundPercent == 0) {
            return 0;
        }
    ...
    function recoverFundsOfAlpha() external onlyOwner() onlyInitialized() returns ( bool ) {
        uint256 _alphaTotalFunds = alphaMarket.totalFunds();
        uint256 _fundsForTeamFromAlpha = _alphaTotalFunds.mul(teamFundPercent).div(100);
        principle.safeTransfer( 
            address(alphaMarket), 
            min(
                _alphaTotalFunds, 
                alphaLiquidFunds().add(alphaMintFunds()).add(_fundsForTeamFromAlpha)
            )
        );
        return true;
    }

    function recoverFundsOfPublic() external onlyOwner() onlyInitialized() returns ( bool ) {
        uint256 _publicTotalFunds = publicMarket.totalFunds();
        uint256 _fundsForTeamFromPublic = _publicTotalFunds.mul(teamFundPercent).div(100);
        principle.safeTransfer( 
            address(publicMarket), 
            min(
                _publicTotalFunds, 
                publicLiquidFunds().add(publicMintFunds()).add(_fundsForTeamFromPublic)
            )
        );
        return true;
    }
